...
from ref.coverage import unaddressed_comments  # noqa: E402
...
PARSER_LINES = [
    ...
    "```JSONL",
    "```",
    "Omitted: AR_PARA_9 lacked new_text.",
...
SESSION_CASES = [
    ...
    # fence handling (Goal 4)
    ("fenced_valid", TOKEN, ["```jsonl", _meta(2), EDIT, EDIT, "```"]),
    ("fenced_language_tag_upper", TOKEN, ["```JSONL", _meta(1), EDIT, "```"]),
    ("fenced_prose_after_close", TOKEN, ["```jsonl", _meta(1), EDIT, "```", "Omitted: AR_PARA_9 lacked new_text."]),
    ("fenced_blanks_inside", TOKEN, ["```jsonl", _meta(2), "", EDIT, "  ", EDIT, "```"]),
    ("fence_only_no_payload", TOKEN, ["```jsonl", "```"]),
    ("unclosed_fence_takes_rest", TOKEN, ["```jsonl", _meta(1), EDIT]),
    ("no_fence_raw_paste", TOKEN, [_meta(1), EDIT]),
]

# --------------------------------------------------------------------------
# Coverage cases: (name, comment_ids list, edits list of (change_type, bid))
# --------------------------------------------------------------------------

COVERAGE_CASES = [
    ("all_addressed", ["AR_COMMENT_1", "AR_COMMENT_2"],
     [("reply_to_comment", "AR_COMMENT_1"), ("add_comment_only", "AR_COMMENT_2")]),
    ("one_missing", ["AR_COMMENT_1", "AR_COMMENT_2", "AR_COMMENT_3"],
     [("reply_to_comment", "AR_COMMENT_1"), ("reply_to_comment", "AR_COMMENT_3")]),
    ("none_addressed", ["AR_COMMENT_1", "AR_COMMENT_2"], []),
    ("no_comments", [], [("replace_text", "AR_PARA_1")]),
    ("addressing_noncomment_target", ["AR_COMMENT_1"], [("add_comment_only", "AR_PARA_5")]),
    ("replace_on_comment_not_addressing", ["AR_COMMENT_1"], [("replace_text", "AR_COMMENT_1")]),
    ("revision_on_comment_not_addressing", ["AR_COMMENT_1"], [("accept_revision", "AR_COMMENT_1")]),
    ("duplicate_addressing_ok", ["AR_COMMENT_1", "AR_COMMENT_2"],
     [("reply_to_comment", "AR_COMMENT_1"), ("reply_to_comment", "AR_COMMENT_1"), ("reply_to_comment", "AR_COMMENT_2")]),
    ("order_preserved", ["AR_COMMENT_3", "AR_COMMENT_1", "AR_COMMENT_2"], []),
    ("case_insensitive_ct", ["AR_COMMENT_1"], [("Reply_To_Comment", "AR_COMMENT_1")]),
]
...
def build_coverage_vectors() -> str:
    lines = [
        "# AutoReviewer golden vectors: comment-coverage suite. GENERATED by tests/vectors/regenerate.py -- do not hand-edit.",
        "# Format: comment_ids_csv <TAB> edits (ct:bid; ...) <TAB> unaddressed_csv <TAB> status",
        "# A comment is addressed iff a reply_to_comment/add_comment_only edit targets its id.",
        "# Empty edits/unaddressed columns are literally empty; status is ALL_ADDRESSED | UNADDRESSED.",
    ]
    for name, comment_ids, edits in COVERAGE_CASES:
        missing, status = unaddressed_comments(comment_ids, edits)
        edits_s = ";".join("%s:%s" % (ct, bid) for ct, bid in edits)
        cols = [
            ",".join(comment_ids),
            edits_s,
            ",".join(missing),
...
def main():
    files = {
        ...
        "coverage_vectors.txt": build_coverage_vectors(),
    }
